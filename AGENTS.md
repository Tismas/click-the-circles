...
- Prefer string union over enums, if you also need an array of values - base the string union on that array (const arr = ["a", "b"] as const; type MyUnion = typeof arr[number])
